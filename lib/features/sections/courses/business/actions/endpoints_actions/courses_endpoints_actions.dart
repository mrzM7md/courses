...
class CoursesEndpointsActions implements BaseCoursesEndpointsActions {
  ...
  @override
  Future<Either<ErrorModel, SuccessModel<UnitModel>>> addEditUnit({required UnitModel  unitModel}) async {
    try{
      Response response = unitModel.id == null ? await ApiConstance.postData(
          url: ApiConstance.httpLinkCreateUnit, data: unitModel.toJson(), accessToken: ""
      ) :
    await ApiConstance.putData(url: ApiConstance.httpLinkUpdateUnit, accessToken: "", data: unitModel.toJson());

    dynamic jsonData = jsonDecode(response.body);
    if(response.statusCode >= 200 && response.statusCode < 300) {
    UnitModel categoryModel = UnitModel.fromJson(json: jsonData['data']);
    SuccessModel<UnitModel> successModel = SuccessModel<UnitModel>(statusCode: response.statusCode, message: jsonData['message'], data: categoryModel);
    return Right(successModel);
    }
  ...
  @override
  Future<Either<ErrorModel, SuccessModel<String?>>> deleteUnit({required int unitId}) async {
    try{
      Response response = await ApiConstance.deleteData(
          url: ApiConstance.httpLinkDeleteUnit(unitId: unitId),
          accessToken: ""
      );
